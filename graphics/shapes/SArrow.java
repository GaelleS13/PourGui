...
public class SArrow extends Shape {
	...
	public void grow() {
		height+=5;
		width+=5;
		int xloc[] = { loc.x, width / 2 + loc.x, width / 2 + loc.x, width + loc.x, width / 2 + loc.x, width / 2 + loc.x,
				+loc.x };
		int yloc[] = { height / 4 + loc.y, height / 4 + loc.y, loc.y, height / 2 + loc.y, height + loc.y,
				3 * height / 4 + loc.y, 3 * height / 4 + loc.y };
		this.poly = new Polygon(xloc, yloc, 7);
